...
#### Right Column (Visual)
- The image/illustration in the right column **changes in sync with each carousel dot** — every dot click or auto-advance swaps the right-side visual with a transition animation
- **Transition:** Crossfade or slide-in from right (300–400ms ease-in-out)
- Each slide has its own unique right-column visual pairing:

| Dot | Slide | Right Column Visual |
|---|---|---|
| ● 1 | Grow Your Green Space | 3D Monstera in terracotta pot + cute smiley watering can mid-pour, soft green blob background, floating leaves |
| ○ 2 | Flower Plants | 3D red rose or blooming flower illustration, warm pink/orange gradient blob background |
| ○ 3 | Indoor Plants | 3D snake plant or fiddle-leaf fig in a white ceramic pot, muted sage green blob background |
| ○ 4 | Balcony Decor | 3D hanging planter basket with colorful flowers, light sky-blue blob background |
| ○ 5 | AI Smart Care | 3D AI robot (green bot with leaf ears) holding a small plant, soft teal blob background |

- **Background circle/blob:** Color shifts subtly per slide to complement each visual
- **Floating leaves:** Present on all slides, vary slightly in density/position per slide
- **Decorative sparkles (✦):** Repositioned per slide for visual freshness

#### Carousel Dots
- 5 dot indicators at bottom center, below the two-column layout
- Clicking any dot simultaneously updates **both** the left column text content AND the right column visual
- **Active dot:** Elongated dark green pill shape (width animates from circle → pill on activation)
- **Inactive dots:** Small grey/beige circles
- **Auto-advance:** Slides cycle automatically every 4–5 seconds; pauses on hover
- **Transition type:** Right column uses crossfade; left column text uses fade-up (opacity + translateY)
...
| **Carousel dots** | Click / Auto | Active dot morphs circle → pill (width transition 250ms); triggers right column visual crossfade + left column text fade-up simultaneously |
| **Hero right column visual** | Dot change / Auto-advance | Crossfade between slide images (opacity 0→1, 350ms ease-in-out); previous image fades out concurrently |
| **Hero left column text** | Dot change / Auto-advance | Heading + subtext fade up (opacity 0→1 + translateY 12px→0, 300ms ease) |
| **Hero carousel** | Auto-advance | Cycles every 4–5s; pauses on mouse hover over hero section |
