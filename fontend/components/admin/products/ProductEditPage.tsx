...
export function ProductEditPage({ mode, productId, initialData }: Props) {
  ...
  useEffect(() => {
    if (fetchedProduct && !isNew) {
      setData(prev => ({
        ...
        costPerUnit: fetchedProduct.cost_price ? String(fetchedProduct.cost_price) : '',
        priceNote: fetchedProduct.price_note || '',
        isTaxable: fetchedProduct.is_taxable ?? true,
        taxRate: fetchedProduct.tax_rate ? `GST ${parseFloat(String(fetchedProduct.tax_rate))}%` : 'GST 18%',
        ...
        deliveryEta: fetchedProduct.delivery_eta_label || '3–5 business days',
        healthGuarantee: fetchedProduct.health_guarantee_label || '7-day health guarantee',
        packagingLabel: fetchedProduct.packaging_label || 'Eco-friendly packaging',
        freeDelivery: fetchedProduct.free_delivery_eligible ?? true,
        weight: fetchedProduct.weight_grams ? String(fetchedProduct.weight_grams) : '',
        dimLength: fetchedProduct.length_cm ? String(fetchedProduct.length_cm) : '',
        dimWidth: fetchedProduct.width_cm ? String(fetchedProduct.width_cm) : '',
        dimHeight: fetchedProduct.height_cm ? String(fetchedProduct.height_cm) : '',
  ...
  const saveProduct = async (status: ProductStatus) => {
    ...
    const payload = {
      ...
      dimWidth: data.dimWidth,
      dimHeight: data.dimHeight,
