...
export function useCustomerReviews(uuid: string) { return useQuery({ queryKey: [...detailKey(uuid), "reviews"], queryFn: () => api.getCustomerReviewsApi(uuid), enabled: Boolean(uuid) }); }
export function useCustomerActivity(uuid: string) { return useQuery({ queryKey: [...detailKey(uuid), "activity"], queryFn: () => api.getCustomerActivityApi(uuid), enabled: Boolean(uuid) }); }
export function useCustomerCart(uuid: string) { return useQuery({ queryKey: [...detailKey(uuid), "cart"], queryFn: () => api.getCustomerCartApi(uuid), enabled: Boolean(uuid) }); }
export function useCustomerWishlist(uuid: string) { return useQuery({ queryKey: [...detailKey(uuid), "wishlist"], queryFn: () => api.getCustomerWishlistApi(uuid), enabled: Boolean(uuid) }); }
export function useAllActivityLogs(filters: Parameters<typeof api.getAllActivityLogsApi>[0] = {}) { return useQuery({ queryKey: ["admin-activity-logs", filters], queryFn: () => api.getAllActivityLogsApi(filters), staleTime: 10_000 }); }
